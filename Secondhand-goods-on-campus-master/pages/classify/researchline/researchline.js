...
wx.setClipboardData({
  ...
  onLoad: function (options) {
    var that=this
    var teacherDetailId=options.id
       that.setData({
        teacherDetailId:teacherDetailId,
        type:options.type
       },()=>{
         if(that.data.type == 0){
          that.teacherDetail(that.data.teacherDetailId)
         }else{
          that.studentDetail(that.data.teacherDetailId)
...
/*   teacherDetail(id){
  ...
  teacherDetail(id){
    let that = this;
    db.collection("TeacherData").doc(id).get({
      success(res){ 
       console.log("数据获取成功啦",res);
...
studentDetail(id){
  let that = this;
  db.collection("StudentData").doc(id).get({
    success(res){ 
     console.log("数据获取成功啦",res);
     var newsInf=res.data    
      var start=that.timeToDate(res.data.time)
